...
export class AccountService {
  ...
  login(email, password) {
    return this.http
      .post<User>(`${environment.apiUrl}/profile/authenticate`, {
        email,
        password,
      })
      .pipe(
  ...
  update(id, params) {
    return this.http.put(`${environment.apiUrl}/profile/${id}`, params).pipe(
      map((x) => {
  ...
  delete(id: string) {
    return this.http.delete(`${environment.apiUrl}/profile/${id}`).pipe(
      map((x) => {
